    ...
    internal class EmitterWriterBuilder
    {
        ...
        private EmitWriterAction<T> CreateEmitter<T>(Type type, BuilderContext state)
        {
            ...
            if (type.IsArray)
            {
                ...
                return (EmitWriterAction<T>)s_createArrayEmitter.MakeGenericMethod(elem).Invoke(this, [state]);
            }
            else if (type.IsConstructedGenericType)
            {
                ...
                else if (def == typeof(List<>) || def == typeof(IList<>) || def == typeof(IReadOnlyList<>) || def == typeof(IEnumerable<>))
                {
                    ...
                    return (EmitWriterAction<T>)s_createListEmitter.MakeGenericMethod(elem, type).Invoke(this, [state]);
                }
                else if (def == typeof(Dictionary<,>) || def == typeof(IDictionary<,>) || def == typeof(IReadOnlyDictionary<,>))
                {
                    ...
                    return (EmitWriterAction<T>)s_createAnyObjectEmitter.MakeGenericMethod(valueType, type).Invoke(this, [state]);
                }
                else
                {
