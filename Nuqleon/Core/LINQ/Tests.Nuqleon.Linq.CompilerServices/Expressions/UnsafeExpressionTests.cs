...
namespace Tests.System.Linq.CompilerServices
{
...
#endif
    {
        ...
        public void ExpressionFactory_Block()
        {
            ...
            AssertExpr(f => f.Block(TypeOf(typeof(double)), new ParameterExpression[] { p1 }, [stmt1, stmt2, stmt3, stmt4]));
            AssertExpr(f => f.Block(TypeOf(typeof(double)), new List<ParameterExpression> { p1 }, [stmt1, stmt2, stmt3, stmt4]));
            ...
            AssertExpr(f => f.Block(new ParameterExpression[] { p1 }, [stmt1, stmt2, stmt3, stmt4]));
            AssertExpr(f => f.Block(new List<ParameterExpression> { p1 }, [stmt1, stmt2, stmt3, stmt4]));
        }
...
<<<<<<< TODO: Unmerged change from project 'Tests.Nuqleon.Linq.Expressions.Bonsai', Before:
...
=======
        ...
        public void ExpressionFactory_New()
        {
            ...
            AssertExpr(f => f.New(c, new Expression[] { s, x }, [i, j]));
            AssertExpr(f => f.New(c, new List<Expression> { s, x }, [i, j]));
...
#pragma warning disable IDE0060 // Remove unused parameter
        private sealed class A0 { }
        private sealed class A1(int a)
        {
        }
        private sealed class A2(int a, int b)
        {
        }
        private sealed class A3(int a, int b, int c)
        {
        }
        private sealed class A4(int a, int b, int c, int d)
        {
        }
        private sealed class A5(int a, int b, int c, int d, int e)
        {
        }
        private sealed class A6(int a, int b, int c, int d, int e, int f)
        {
        }
#pragma warning restore IDE0060 // Remove unused parameter
