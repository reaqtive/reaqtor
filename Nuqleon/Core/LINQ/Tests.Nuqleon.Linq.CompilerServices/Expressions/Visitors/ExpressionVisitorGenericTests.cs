...
public class ExpressionVisitorGenericTests
{
    ...
    private class MyVisitor : ExpressionVisitor<Expression, LambdaExpression, ParameterExpression, NewExpression, ElementInit, MemberBinding, MemberAssignment, MemberListBinding, MemberMemberBinding, CatchBlock, SwitchCase, LabelTarget>
    {
        public void TestArgumentChecks()
        {
            ...
            var ex2 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitBlock(node: null));
            ...
            var ex4 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitConditional(node: null));
            Assert.AreEqual("node", ex4.ParamName);
            var ex5 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitConstant(node: null));
            Assert.AreEqual("node", ex5.ParamName);
            var ex6 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitDebugInfo(node: null));
            Assert.AreEqual("node", ex6.ParamName);
            var ex7 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitDefault(node: null));
            Assert.AreEqual("node", ex7.ParamName);
            var ex8 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitDynamic(node: null));
            Assert.AreEqual("node", ex8.ParamName);
            var ex9 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitElementInit(node: null));
            Assert.AreEqual("node", ex9.ParamName);
            var ex10 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitGoto(node: null));
            Assert.AreEqual("node", ex10.ParamName);
            var ex11 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitIndex(node: null));
            Assert.AreEqual("node", ex11.ParamName);
            var ex12 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitInvocation(node: null));
            Assert.AreEqual("node", ex12.ParamName);
            var ex13 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitLabel(node: null));
            Assert.AreEqual("node", ex13.ParamName);
            var ex14 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitLabelTarget(node: null));
            Assert.AreEqual("node", ex14.ParamName);
            var ex15 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitLambda<Action>(node: null));
            Assert.AreEqual("node", ex15.ParamName);
            var ex16 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitListInit(node: null));
            Assert.AreEqual("node", ex16.ParamName);
            var ex17 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitLoop(node: null));
            Assert.AreEqual("node", ex17.ParamName);
            var ex18 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMember(node: null));
            Assert.AreEqual("node", ex18.ParamName);
            var ex19 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMemberAssignment(node: null));
            Assert.AreEqual("node", ex19.ParamName);
            var ex20 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMemberBinding(node: null));
            Assert.AreEqual("node", ex20.ParamName);
            var ex21 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMemberInit(node: null));
            Assert.AreEqual("node", ex21.ParamName);
            var ex22 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMemberListBinding(node: null));
            Assert.AreEqual("node", ex22.ParamName);
            var ex23 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMemberMemberBinding(node: null));
            Assert.AreEqual("node", ex23.ParamName);
            var ex24 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitMethodCall(node: null));
            Assert.AreEqual("node", ex24.ParamName);
            var ex25 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitNew(node: null));
            Assert.AreEqual("node", ex25.ParamName);
            var ex26 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitNewArray(node: null));
            Assert.AreEqual("node", ex26.ParamName);
            var ex27 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitParameter(node: null));
            Assert.AreEqual("node", ex27.ParamName);
            var ex28 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitRuntimeVariables(node: null));
            Assert.AreEqual("node", ex28.ParamName);
            var ex29 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitSwitch(node: null));
            Assert.AreEqual("node", ex29.ParamName);
            var ex30 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitSwitchCase(node: null));
            Assert.AreEqual("node", ex30.ParamName);
            var ex31 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitTry(node: null));
            Assert.AreEqual("node", ex31.ParamName);
            var ex32 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitTypeBinary(node: null));
            Assert.AreEqual("node", ex32.ParamName);
            var ex33 = Assert.ThrowsExactly<ArgumentNullException>(() => base.VisitUnary(node: null));
            Assert.AreEqual("node", ex33.ParamName);
        }
