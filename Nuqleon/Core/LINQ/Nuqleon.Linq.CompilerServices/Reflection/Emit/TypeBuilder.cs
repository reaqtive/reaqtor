...
namespace Nuqleon.Reflection.Emit
{
    public class TypeBuilder : Type
    {
        ...
        private static readonly Func<object, string, MethodAttributes, CallingConventions, Type, Type[], MethodBuilder> s_DefineMethod;
        ...
        static TypeBuilder()
        {
            ...
            var method_System_Reflection_Emit_TypeBuilder_DefineMethod = type_System_Reflection_Emit_TypeBuilder.GetMethod(nameof(DefineMethod), new[] { typeof(string), typeof(MethodAttributes), typeof(CallingConventions), typeof(Type), typeof(Type[]) });
            ...
            s_DefineMethod = Expression.Lambda<Func<object, string, MethodAttributes, CallingConventions, Type, Type[], MethodBuilder>>(
                Expression.New(
                    typeof(MethodBuilder).GetConstructor(new[] { typeof(object) }),
                    Expression.Call(
                        typeBuilder,
                        method_System_Reflection_Emit_TypeBuilder_DefineMethod,
                        name,
                        methodAttributes,
                        callingConventions,
                        type,
        ...
        public override Type GetElementType() => throw new NotSupportedException(); // NB: Same as BCL code.
        ...
        protected override PropertyInfo GetPropertyImpl(string name, BindingFlags bindingAttr, Binder binder, Type returnType, Type[] types, ParameterModifier[] modifiers) => throw new NotSupportedException(); // NB: Same as BCL code.
        ...
        public override bool IsAssignableFrom(Type c) => _type.IsAssignableFrom(c.Unwrap());

        public override bool IsSubclassOf(Type c) => _type.IsSubclassOf(c.Unwrap());
        ...
        public MethodBuilder DefineMethod(string name, MethodAttributes attributes, Type returnType, Type[] parameters) => DefineMethod(name, attributes, CallingConventions.Standard, returnType, parameters);
        public MethodBuilder DefineMethod(string name, MethodAttributes attributes, CallingConventions conventions, Type returnType, Type[] parameters) => s_DefineMethod(_type, name, attributes, conventions, returnType.Unwrap(), parameters.Unwrap());
        ...
        //     - SetParent
        //
        //     as well as some overloads for various Define methods.
        //
        //     Due to the lack of support for generic types, the following are not overridden and throw:
        //
        //     - GetGenericArguments
