...
public class TimelineObservable<T> : SubscribableBase<T>
{
    ...
    private abstract class BaseTimelineSubscription : StatefulOperator<TimelineObservable<T>, T>
    {
        ...
        private static T SetupInnerSubscribable(INotification<string> @event)
        {
            ...
            var timelineCtor = timelineType.GetConstructor([typeof(Uri), typeof(bool)]);
            ...
            var quotedSubscribableCtor = quotedSubscribableType.GetConstructor([typeof(Expression)]);
            return (T)quotedSubscribableCtor.Invoke([subscribableExpression]);
        }
