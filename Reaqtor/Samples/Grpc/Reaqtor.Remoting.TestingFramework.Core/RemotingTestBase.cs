...
namespace Reaqtor.Remoting.TestingFramework
{
    ...
    // NB: the archived RemotingTestBase selected the platform via an #if APPDOMAINPLATFORM ... #else ... #endif
    //     between AppDomainTestPlatform and InMemoryTestPlatform. The AppDomain platform is not ported on net10.0
    //     (AppDomainTestPlatform / Reaqtor.Remoting.Platform.AppDomain are dropped), so only the InMemory aliases
    //     remain and the conditional is removed.
    ...
    public abstract class RemotingTestBase
    {
        ...
        private static readonly Lazy<TestReactiveEnvironment> s_environment = new(() =>
        {
            ...
            new ReactivePlatformDeployer(platform, new TestDeployable()).Deploy();
            ...
            where TContext : ReactiveClientContext
        {
            ...
            // NB: Reactor.*/DomainFeeds deployables are out of 0a scope (plan §10, Milestone 2+) and unported; only
            //     TestDeployable (a CoreDeployable) is deployed. See remark in the s_environment factory above.
            new ReactivePlatformDeployer(platform, new TestDeployable()).Deploy();

            var scheduler = (ITestScheduler)platform.QueryEvaluators.First().Scheduler;
            // NB: the archived test base used new AgentsClient(platform) from Reaqtor.Remoting.Reactor.Client.Library
            //     (an unported, out-of-scope Reactor project) purely to obtain a generic ReactiveClientContext to
            //     drive the test body. The ported InMemory platform's CreateClient() returns a ReactivePlatformClient
            //     whose Context is a RemotingClientContext (also a ReactiveClientContext), so it is the faithful
            //     in-scope replacement.
            var client = platform.CreateClient();
            ...
            where TContext : ReactiveClientContext
        {
            ...
            // NB: replaced the unported Reactor AgentsClient with the ported InMemory platform's CreateClient().
            //     See remark in RunVirtualTask above.
            var client = platform.CreateClient();
