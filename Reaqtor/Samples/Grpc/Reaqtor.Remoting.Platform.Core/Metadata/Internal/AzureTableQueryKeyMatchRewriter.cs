...
// NB: ADAPTATION (plan §2.6): removed `using Microsoft.Azure.Cosmos.Table;`. The only Cosmos type referenced was
//     the `TableEntity` base used in the `ReflectionHelpers.InfoOf((TableEntity e) => e.PartitionKey/RowKey)`
//     metadata lookups below; those now resolve to the local Cosmos-free TableEntity in this namespace (see
//     Metadata/Entities/TableEntity.cs), which exposes the same PartitionKey/RowKey properties. No `EdmType` (or
//     any other Cosmos type) is referenced here.
