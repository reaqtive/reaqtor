...
// NB: ADAPTATION (plan §2.6): removed `using Microsoft.Azure.Cosmos.Table;`. The Cosmos `TableRequestOptions`
//     field/ctor-parameter and its `ExponentialRetry` retry policy were dropped (the in-memory/gRPC-Storage backend
//     has no remote retry semantics), and the `TableEntity` parameter on InsertAsync now binds to the local
//     Cosmos-free TableEntity in this namespace. The ITable calls below use the local 1-arg
//     CreateIfNotExistsAsync(state) / 2-arg ExecuteAsync(operation, state) signatures.
...
namespace Reaqtor.Remoting.Metadata
{
    ...
    public class AzureMetadataQueryProvider : IQueryProvider
    {
        ...
        public AzureMetadataQueryProvider(ITableClient tableClient, IStorageResolver storageResolver)
        {
        ...
        public IQueryable CreateQuery(Expression expression)
        {
            ...
            var result = (IQueryable)createQuery.Invoke(this, [expression]);
...
#pragma warning disable CA1054 // URI-like parameters should not be strings. (Legacy approach.)
        ...
        public async Task InsertAsync(Uri uri, TableEntity entity, string metadataCollectionUri)
        {
            ...
            await table.CreateIfNotExistsAsync(null).ConfigureAwait(false);
            var insert = new InsertTableOperation(entity);
            await table.ExecuteAsync(insert, null).ConfigureAwait(false);
        }
        ...
        public async Task DeleteAsync(Uri uri, string metadataCollectionUri)
        {
            ...
            await table.ExecuteAsync(delete, null).ConfigureAwait(false);
        }
...
#pragma warning restore CA1054
        ...
        private object ExecuteRemoteQuery(Expression queryExpression)
        {
            ...
            var resultSet = fetchQueryResults.Invoke(this, [tableAddress, query]);
            ...
            var result = materialize.Invoke(null, [resultSet]);
