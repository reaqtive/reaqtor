...
// NB: ADAPTATION (plan §2.6): removed `using Microsoft.Azure.Cosmos.Table;`. The `TableEntity` upper bound on the
//     TMetadataEntity type parameter now binds to the local Cosmos-free TableEntity in this namespace (see
//     Metadata/Entities/TableEntity.cs), which all metadata entities derive from.
...
namespace Reaqtor.Remoting.Metadata
{
        ...
        where TMetadataInterface : IKnownResource
    {
        ...
        {
            ArgumentNullException.ThrowIfNull(underlyingDictionary);
