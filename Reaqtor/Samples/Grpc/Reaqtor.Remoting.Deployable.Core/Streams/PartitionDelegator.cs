...
namespace Reaqtor.Remoting.Deployable.Streams
{
    internal abstract class PartitionDelegator : IDelegationTarget
    {
        ...
        private Expression DelegateImpl(ParameterExpression node, Expression expression)
        {
            if (expression.NodeType == ExpressionType.Invoke)
            {
                ...
                if (invocation.Arguments[0] == node && invocation.Expression.NodeType == ExpressionType.Parameter && ((ParameterExpression)invocation.Expression).Name == "rx://operators/filter")
                {
                    ...
                    if (PredicateChecker.TryCheck(lambda, out List<PartitionInfo> lst, out Expression expr))
                    {
        ...
        {
            public static bool TryCheck(LambdaExpression lambda, out List<PartitionInfo> infos, out Expression rest)
            {
            ...
            private static void Check(Expression predicate, ParameterExpression parameter, ref List<PartitionInfo> infos, ref Expression rest)
            {
...
#pragma warning restore IDE0034 // Simplify 'default' expression
        ...
        {
            return (Type)s_MakeFuncTypeTR.MakeGenericMethod(T, R).Invoke(null, []);
        }
        ...
        {
            return (Expression)s_defaultComparerExpression.MakeGenericMethod([t]).Invoke(null, []);
        }
