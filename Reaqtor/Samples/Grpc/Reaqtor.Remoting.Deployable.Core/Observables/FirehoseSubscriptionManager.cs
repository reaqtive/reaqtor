...
{
    /// <summary>
    /// Manages all the FireHose subscriptions in-process via the static <see cref="Instance"/> singleton (the
    /// archived implementation described this as "AppDomain level", but it was already a process-wide singleton
    /// backed by <see cref="ConcurrentDictionary{TKey,TValue}"/> rather than an AppDomain data slot; there is no
    /// AppDomain on net10.0). The subscriptions are mapped per topic and then per QE instance Id.
    /// </summary>
