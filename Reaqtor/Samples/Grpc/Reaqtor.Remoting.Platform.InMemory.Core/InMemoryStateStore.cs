...
#pragma warning disable CA1024 // Use properties where appropriate: this snapshots the store (allocates a fresh list each call), so a method is correct - a property would misrepresent it as cheap field access.
...
#pragma warning restore CA1024
    ...
    public bool RemoveItem(string category, string key)
    {
        ...
        {
            removed = values.TryRemove(key, out var value);
        }
        ...
        get
        {
            ...
            foreach (var kv in _internalStore)
            {
                ...
                foreach (var kv2 in kv.Value)
                {
                    ...
                    {
                        // NB: explicit StringComparison.Ordinal for the net10 analyzer (CA1307); behavior-neutral (DebugView only).
                        var bytes = BitConverter.ToString(chunk).Replace("-", " ", StringComparison.Ordinal);
