...
namespace Reaqtor.Remoting.Platform
{
    public abstract class ReactivePlatformClientBase : IReactivePlatformClient
    {
        ...
        // NB (plan §2.5): the archived client->QC leg wrapped the query-coordinator connection in
        //     LocalReactiveServiceConnection (which itself wrapped each command in the .NET Remoting
        //     ReactiveServiceCommandProxy). That deleted-layer type is replaced by the in-process
        //     InProcessReactiveServiceConnection (Reaqtor.Remoting.Protocol, in Reaqtor.Remoting.Core),
        //     which calls the coordinator's IRemotingReactiveServiceConnection directly with no transport.
        protected virtual IReactiveServiceProvider ServiceProvider => new RemotingServiceProvider(new InProcessReactiveServiceConnection(_queryCoordinator), GetRemoteObserver);
