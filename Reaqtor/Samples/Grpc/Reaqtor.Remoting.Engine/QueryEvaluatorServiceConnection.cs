...
#pragma warning restore CA2213, IDE0079
    ...
    public override void Start()
    {
        ...
        // ADAPTATION (plan §3.6, adaptation #6a): the archived QE published the messaging connection into an
        // AppDomain-keyed slot via ServiceInstanceHelpers.SetMessagingServiceInstance(AppDomain.CurrentDomain, ...),
        // from which the archived MessageRouter later read it. There is no AppDomain (nor ServiceInstanceHelpers) on
        // net10.0; the messaging connection is now injected directly into the MessageRouter placed in
        // OperatorContextElements (see below), so this line is removed.
        StartCore();
    }
    ...
    protected virtual Dictionary<string, object> OperatorContextElements =>
        new()
        {
            ...
            // ADAPTATION (plan §3.6, adaptation #6b): the archived MessageRouter() resolved its messaging connection
            // from the AppDomain slot set in Start (see above). On net10.0 the connection is injected via the
            // MessageRouter(IReactiveMessagingConnection) ctor (added in Reaqtor.Remoting.Platform.Core) so in-engine
            // operators can Subscribe/Publish to the broker. GetMessaging returns _configuration.MessagingConnection.
            { MessageRouter.ContextHandle, new MessageRouter(GetMessaging(_configuration)) },
        };
    ...
    {
        // NB (plan §2.6, adaptation #2): functionally unchanged. AzureReactiveMetadata, StorageConnectionTableClient
        // and AzureStorageResolver now come from the Cosmos-free metadata layer in Reaqtor.Remoting.Platform.Core
        // (namespace Reaqtor.Remoting.Metadata; already imported above). ReactiveMetadataCache / ReactiveMetadataInMemory
        // are the ported MetadataCaching types in this assembly. The commented-out MetadataStorageType.Azure
        // (Cosmos.Table) branch is left exactly as the archive has it; it is not supported on net10.0.
