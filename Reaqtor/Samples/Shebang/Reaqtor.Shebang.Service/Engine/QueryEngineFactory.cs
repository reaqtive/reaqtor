...
namespace Reaqtor.Shebang.Service
{
    public static class QueryEngineFactory
    {
        ...
        public static async Task<QueryEngine> CreateNewAsync(IQueryEngineStateStore store, IReadOnlyDictionary<string, object> context = null)
        {
            ...
            var engine = new QueryEngine(new Uri("rx://engine/" + Guid.NewGuid().ToString("D")), sch, store, context);

            var ctx = engine.GetClient();

            await DeployQueryOperators.DefineAsync(ctx);

            await ctx.DefineObserverAsync(new Uri("nucleus://shebang/observers/cout"), ctx.Provider.CreateQbserver<T>(Expression.New(typeof(ConsoleObserver<T>))), null, CancellationToken.None);
            await ctx.DefineObservableAsync<TimeSpan, DateTimeOffset>(new Uri("nucleus://shebang/observables/timer"), t => new TimerObservable(t).AsAsyncQbservable(), null, CancellationToken.None);
