...
namespace Reaqtor.QueryEngine.KeyValueStore.InMemory
{
    ...
    public class InMemoryStateWriter : IStateWriter
    {
        ...
        /// <summary>
        /// Seals the instance against further changes; called when rolling back or committing.
        /// </summary>
        private void Seal()
        {
            var wasSealed = Interlocked.Exchange(ref _sealed, 1);

            if (wasSealed > 0)
            {
                throw AlreadyCommittedOrRolledBack;
            }
        }

        /// <summary>
        /// Checks if the instance has been sealed and further changes should be prevented.
        /// </summary>
        ...
        private static Exception AlreadyCommittedOrRolledBack => new InvalidOperationException("Transaction has already been committed or rolled back.");
