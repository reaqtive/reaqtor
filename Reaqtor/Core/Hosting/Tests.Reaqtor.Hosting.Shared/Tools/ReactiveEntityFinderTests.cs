...
public class ReactiveEntityFinderTests
{
    ...
    public void ReactiveEntityFinder_Test()
    {
        AssertFound(
            ...
            new Result(ReactiveEntityType.Observable, "foo", [[Expression.Constant(4)]])
        );

        AssertFound(
            Expression.Invoke(Expression.Parameter(typeof(Func<int, IAsyncReactiveQbserver<int>>), "foo"), Expression.Constant(4)),
            new Result(ReactiveEntityType.Observer, "foo", [[Expression.Constant(4)]])
        );

        AssertFound(
            Expression.Parameter(typeof(IAsyncReactiveQubject<int>), "foo"),
            new Result(ReactiveEntityType.Stream, "foo", [[]])
        );
        ...
        AssertFound(
            ...
            new Result(ReactiveEntityType.StreamFactory, "foo", [[]])
        );

        AssertFound(
            Expression.Parameter(typeof(IAsyncReactiveQubscriptionFactory), "foo"),
            new Result(ReactiveEntityType.SubscriptionFactory, "foo", [[]])
        );
        ...
        AssertFound(
            ...
            new Result(ReactiveEntityType.Observable, "Observable", [[]]),
            new Result(ReactiveEntityType.Observer, "Observer", [[]])
        );
        ...
        AssertFound(
            ...
            new Result(ReactiveEntityType.Observable, "Observable", [[Expression.Constant(42)]]),
            new Result(ReactiveEntityType.Observer, "Observer", [[Expression.Constant(42)]])
        );
